...
impl Target {
    ...
    pub fn get_rotation_effect(&mut self, transform: Transform, rotation_speed: f32) -> RotationEffect {
        if self.rotation_done { return RotationEffect::RotationDone; }

        if self.is_in_facing_threshold(transform, rotation_speed) {
            self.rotation_done = true;
            return RotationEffect::FinalRotationFix(transform.looking_to(self.direction, Vec3::Y).rotation);
        }

        RotationEffect::AngularVelocity(self.get_needed_angular_velocity(transform) * rotation_speed)
    }

    pub fn get_translation_effect(&mut self, transform: Transform, speed: f32) -> TranslationEffect {
        if self.destination_reached { return TranslationEffect::DestinationReached; }
    ...
    fn has_reached_destination(&self, transform: Transform, speed: f32) -> bool {
        let destination_reached_threshold = speed / LINEAR_THRESHOLD_RATIO;
        transform.translation.distance(self.target) < destination_reached_threshold
    }
